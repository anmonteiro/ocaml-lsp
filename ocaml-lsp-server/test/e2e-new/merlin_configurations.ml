...
let other_uri = DocumentUri.of_path "other.ml"
let other_text_document = TextDocumentIdentifier.create ~uri:other_uri
...
let merlin_configurations ?(text_document = text_document) client =
  let params =
...
let select_merlin_configuration ?(text_document = text_document) client id =
  let params =
...
let merlin_errors ?(text_document = text_document) client =
  let params =
...
let%expect_test "new protocol: selection is per document" =
  let request client =
    let open Fiber.O in
    let textDocument =
      TextDocumentItem.create ~uri:other_uri ~languageId:"ocaml" ~version:0 ~text:source
    in
    let* () =
      Client.notification
        client
    ...
    let* response = merlin_errors client in
    Test.print_result response;
    let+ response = merlin_errors ~text_document:other_text_document client in
    Test.print_result response
  in
  Helpers.test ~extra_env:(extra_env "new") source request;
  [%expect
